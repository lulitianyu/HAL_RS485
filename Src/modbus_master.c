...
// idle callback function; gets called during idle time between TX and RX               // функция обратного вызова в режиме ожидания; вызывается во время простоя между TX и RX
void (*_idle)();
void idle(){
  //uint16_t t35 = 35000/19200;
  HAL_Delay(3);
}

// preTransmission callback function; gets called before writing a Modbus message        // функция обратного вызова preTransmission; вызывается перед написанием сообщения Modbus
void (*_preTransmission)();
void preTransmission(){
    HAL_GPIO_WritePin(MB_USART2_TXEN_GPIO_Port, MB_USART2_TXEN_Pin, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIOC, GPIO_PIN_9, GPIO_PIN_SET);// TODO: :Для отладки, Убрать !!!
}
// postTransmission callback function; gets called after a Modbus message has been sent    // функция обратного вызова postTransmission; вызывается после отправки сообщения Modbus
void (*_postTransmission)();
void 
postTransmission(){
    HAL_GPIO_WritePin(MB_USART2_TXEN_GPIO_Port, MB_USART2_TXEN_Pin, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(GPIOC, GPIO_PIN_9, GPIO_PIN_RESET);// TODO: :Для отладки, Убрать !!!
}
...
// eliminate this function in favor of using existing MB request functions // исключаем эту функцию в пользу использования существующих функций запроса MB
uint8_t ModbusMaster_requestFrom(uint16_t address, uint16_t quantity)
{
  uint8_t read;
  // clamp to buffer length   // зажим до длины буфера
  if (quantity > ku8MaxBufferSize)
  {
    quantity = ku8MaxBufferSize;
  }
  // set rx buffer iterator vars   // устанавливаем переменные буфера rx
...
@param u8Index index of transmit buffer array (0x00..0x3F) 0-63
...
{
  uint16_t testTime = Modbus_Master_Millis();
  uint8_t u8ModbusADU[256];
  volatile  uint8_t u8ModbusADUSize = 0;
  //uint8_t u8ModbusADUSize = 0;
  uint8_t i, u8Qty;
  uint16_t u16CRC;
  uint32_t u32StartTime;
  volatile  uint8_t u8BytesLeft = 8;
  ...
  // последовательный порт отправляет данные
  Modbus_Master_Write(u8ModbusADU, u8ModbusADUSize);
  u8ModbusADUSize = 0;
  // ПРОФИ:Нужно реализовать!!!!
      postTransmission();
  // loop until we run out of time or bytes, or an error occurs цикл, пока у нас не закончится время или байты, или пока не произойдет ошибка
  ...
  while (u8BytesLeft && !u8MBStatus)
  {
    ...
    // evaluate slave ID, function code once enough bytes have been read вычисление идентификатора ведомого, код функции будет прочитан достаточно раз
    if (u8ModbusADUSize == 7)
    {
      // verify response is for correct Modbus slave проверить ответ для правильного ведомого Modbus
      ...
      // verify response is for correct Modbus function code (mask exception bit 7) проверка ответа для правильного кода функции Modbus (исключение маски, бит 7)
      ...
      // check whether Modbus exception occurred; return Modbus Exception Code // проверить, произошло ли исключение Modbus; вернуть код исключения Modbus
      ...
      // evaluate returned Modbus function code оцениваем возвращенный код функции Modbus
      ...
      case ku8MBWriteSingleRegister:
      case ku8MBWriteMultipleRegisters:
        u8BytesLeft = 3;//TODO адрес, значение и црц не 1, а 2 байта, поэтому u8BytesLeft = 6;
        break;
  ...
  // verify response is large enough to inspect further  проверка ответа в основном достаточна для дальнейшей проверки
  ...
  // disassemble ADU into words  разбираем ADU на слова
  if (!u8MBStatus)
  {
    // evaluate returned Modbus function code оцениваем возвращенный код функции Modbus
    switch (u8ModbusADU[1])
    ...
    case ku8MBReadCoils:
    case ku8MBReadDiscreteInputs:
      // load bytes into word; response bytes are ordered L, H, L, H, ... загрузить байты в слово; ответные байты упорядочены L, H, L, H, ...
      ...
      // in the event of an odd number of bytes, load last byte into zero-padded word в случае нечетного числа байтов загружаем последний байт в заполненное нулями слово
      if (u8ModbusADU[2] % 2)
      {
        if (i < ku8MaxBufferSize)
        {
    ...
    case ku8MBReadHoldingRegisters:
    case ku8MBReadWriteMultipleRegisters:
      // load bytes into word; response bytes are ordered H, L, H, L, ... // загружаем байты в слово; ответные байты упорядочены H, L, H, L, ...
      for (i = 0; i < (u8ModbusADU[2] >> 1); i++)
      {
        if (i < ku8MaxBufferSize)
        {
  ...
  testTime += (Modbus_Master_Millis() - testTime);
  _u8TransmitBufferIndex = 0;
  u16TransmitBufferLength = 0;
  _u8ResponseBufferIndex = 0;
  return u8MBStatus;
...
*/
/**
Функция Modbus 0x03 Чтение регистров удержания.
Этот код функции используется для чтения содержимого непрерывного блока 
Хранение регистров на удаленном устройстве. Запрос указывает на запуск 
Адрес регистра и количество регистров. Регистры адресуются 
Начиная с нуля.
Данные регистра в буфере ответа упаковываются как одно слово в 
зарегистрироваться.
@param u16ReadAddress адрес первого регистра хранения (0x0000..0xFFFF)
@param u16ReadQty количество регистров хранения для чтения (1..125, принудительно устанавливается удаленным устройством)
@ возвратить 0 при успехе; номер исключения при ошибке
регистрация @ingroup
*/
...
Функция Modbus 0x06 Запись одного регистра.
Этот код функции используется для записи одного регистра хранения в 
Удаленное устройство. В запросе указывается адрес регистра 
Письменные. Регистры являются главными, начиная с нуля.
@param u16WriteAddress адрес регистра хранения (0x0000..0xFFFF)
@param u16WriteValue значение для записи в регистр хранения (0x0000..0xFFFF)
@ возвратить 0 при успехе; номер исключения при ошибке
регистрация @ingroup
*/
...
Функция Modbus 0x0F Запись нескольких катушек.
Этот код функции используется, чтобы заставить каждую катушку в последовательности катушек 
Либо ВКЛ, либо ВЫКЛ на удаленном устройстве. Запрос указывает катушку 
Ссылки должны быть принудительными. Катушки являются главными, начиная с нуля.
Запрошенные состояния ВКЛ / ВЫКЛ определяются содержанием передачи 
Buffer. Логическая «1» в битовой позиции буфера запрашивает 
Логический «0» требует, чтобы он был выключен.
@param u16WriteAddress адрес первой катушки (0x0000..0xFFFF)
@param u16BitQty количество записываемых катушек (1..2000, принудительно устанавливается удаленным устройством)
@ возвратить 0 при успехе; номер исключения при ошибке
@ingroup дискретный
*/
